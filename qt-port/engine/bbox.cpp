...
{
    return ((pt->x >= bbox->left) && (pt->x <= bbox->right) &&
            (pt->y >= bbox->bottom) && (pt->y <= bbox->top));
}
...
{
    return ((pt->x + tol >= bbox->left) && (pt->x - tol <= bbox->right) &&
            (pt->y + tol >= bbox->bottom) && (pt->y - tol <= bbox->top));
}
